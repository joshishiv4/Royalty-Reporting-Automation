...
| Vercel ALERT SWEEP route — standing conditions, on its own cron so it cannot share the sync's fate | [`api/alerts.ts`](../api/alerts.ts) |
...
| `0046` | `creation` — a file a student uploaded. Exists because Supabase Storage has no `student_id` and so cannot say whose file is whose: without it every student sees every student's work. It is also the only place the **original filename** survives, since the object is named with a uuid precisely so a caller cannot overwrite somebody else's. Holds the storage *path*, never a signed URL — those expire, and a stored one is a dead link tomorrow. `uploaded_at` is nullable on purpose: the row is written when we sign and filled when the file lands, so an abandoned upload stays **countable** instead of leaving an orphan object nothing knows about |
| `0047` | The eleven portal-owned tables move to their own schema, **`app`**. `public` keeps the WellnessLiving mirror the sync writes. The separation buys one concrete thing and it is not tidiness: PostgREST is told which schemas to expose, so with one schema, exposing the portal exposes `purchase`, `pay_transaction` and `raw_wl` beside it. **No foreign key is dropped to buy the move** — keys, triggers and views all cross schemas, and `SET SCHEMA` carries a table's indexes, constraints, FKs, triggers and RLS policies with it. What it does *not* carry is the thing that makes this dangerous: a plpgsql body resolves its table names when it **runs**, so moving a table neither errors nor warns, and the break arrives on the next INSERT as `relation "public.cohort_link" does not exist`, raised inside a trigger on `public.session` and failing the sync's own write. That is why the move and the re-pointing of all **fourteen** projection functions are one file. The functions stay in `public`, as does `set_updated_at`, which the mirror shares. `src/` is untouched — the sync writes `person`, `session` and `attendance`, all of which stay put, and never addresses one of the eleven |
| `0048` | `organization` / `organization_membership`, created **in `app`** (so this must run after `0047`), plus `organization_id` on the **seven application tables**. Spin becomes **Organization #1** instead of being the whole system. The WL mirror and the three link tables are untouched: `k_business` already sits on 23 tables holding one value, but it is *WellnessLiving's* key, and a second organization must not need a WL account to exist — WL-shaped rows resolve through `organization.wl_k_business` instead. Membership points at `identity`, never `person` (`person.uid` is WL's and `NOT NULL`, so only people WL knows about could belong). `identity` itself gets no `organization_id`: it is the mapping row and the canonical human, and the same human may later be a participant at one provider and a teacher at another. New rows are filled by `organization_stamp()`, a BEFORE INSERT trigger using `select … into strict` — so the day a second organization exists, an insert that did not choose one **fails loudly** instead of filing it under Spin. RLS enabled with no policies deliberately: `auth_user_id` is null on all 1,292 rows, so no request has ever authenticated |
| `0049` | the same reconcile as `0045`, against `app`. A separate file rather than an edit, because `0045` is still correct where it sits: replayed in order it runs *before* `0047`, when the eleven tables really are in `public`. **`0049` is the one to run**; `0045` is history |
