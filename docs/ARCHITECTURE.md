...
| `0030` | `attendance.dt_checkin_utc` — WL's `dt_register`, "the date the client checked in", present on 55 of 55 sampled records and previously discarded. Evidence beside the verdict: nothing derives from it, and counting its nulls per service type is how Q9 (is check-in used consistently for private lessons) gets answered. Adds `visit_unresolved_past` — sessions that ran while WL still says BOOK, which `visit_awaiting_staff` misses because WL is not asking anybody |
