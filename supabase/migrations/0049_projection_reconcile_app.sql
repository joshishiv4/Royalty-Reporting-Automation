-- =============================================================================
-- 0049  Reconcile the projections, in the app schema
--
-- WHY THIS FILE EXISTS RATHER THAN AN EDIT TO 0045
--   0045 is the same reconcile against public. It is still correct where it
--   sits: replayed in order on a fresh database it runs BEFORE 0047, when the
--   eleven tables really are in public. Rewriting it in place would make a
--   sequential replay fail on a schema that does not exist yet.
--
--   So 0045 is history and this is the one to run. Everything else about it is
--   unchanged - see 0045's header for what a reconcile is for, why every
--   statement is NOT EXISTS driven, and why it reports.
--
...
begin
  ...
  raise notice '0049.1  identities created for people that had none: %', v_count;
  ...
  raise notice '0049.2  identities whose role was re-sorted: %', v_count;
  ...
  raise notice '0049.3  sessions projected that had no link: %', v_count;
  ...
  for r in
    select ss.k_period, ss.dt_start_utc, ss.k_staff
      ...
      join app.identity i on i.k_staff = ss.k_staff and i.teacher_id is not null
  ...
  raise notice '0049.4  teachers attached to sessions: %', v_count;
end;
$$;
...
declare
  v_count integer := 0;
begin
  if to_regclass('app.attendance_record') is null then
    raise notice '0049.5  skipped - attendance_record does not exist (0043/0044 not applied)';
    return;
  end if;
  ...
  raise notice '0049.5  attendance records created that were missing: %', v_count;
  ...
  get diagnostics v_count = row_count;
  raise notice '0049.6  attendance links created that were missing: %', v_count;
end;
$$;
